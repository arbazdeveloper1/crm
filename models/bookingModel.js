...
export const Price_Description = async(newTotalAmountQuoted,newMCO_Description,newAirlineName,newTypeOfCharge,newAirlineCost) =>{
    try {
        const sql = await query("INSERT INTO form_data (total_amount = ? ,mco_description = ?,airline_names =?,charge_type = ?,airline_cost = ?",[newTotalAmountQuoted,newMCO_Description,newAirlineName,newTypeOfCharge,newAirlineCost]);
        return sql;
    } catch (error) {
