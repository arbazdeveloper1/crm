...
export const TrackIp = async (req, res) => {
  try {
    ...
    // Update signed document in form_data table
    await query(
      `update form_data set signed_document = '${fileName}' where customer_id = '${customer_id || "no file available"}'`
    );
    ...
    console.error("Error in TrackIp:", error); // Logs the error instead of crashing the server
    if (!res.headersSent) {
